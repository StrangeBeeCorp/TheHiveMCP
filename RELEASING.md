...
Published accuracy and security evidence (in
[`docs/evaluation/`](docs/evaluation/)) must stay honest as the server evolves.
...
- **Exception — no-behavior-change releases.** If a version contains only
  ...
  in the evaluation report and the release notes. **When in doubt, re-run.**

The evaluation set (candidate models and the pinned judge model) is defined in
the ADR.

## Release notes

Every release ships **curated, categorized release notes** so adopters can
decide whether to upgrade without reading the commit log. Each release sorts
its changes into:

- **User-facing changes** — new tools, capabilities, behavior changes.
- **Breaking changes** — anything requiring action to upgrade, with upgrade
  notes.
- **Security fixes** — called out explicitly.

> Today the release workflow sets `generate_release_notes: true`, which produces
> an uncurated dump of PR titles. Replacing that with a curated mechanism (and
> backfilling notes for past versions) is tracked separately; until then,
> curate the generated notes by hand against the categories above before
> publishing.
...
- [ ] **Re-test policy applied** — either re-ran accuracy + security and updated
      [`docs/evaluation/`](docs/evaluation/), **or** confirmed this is a
