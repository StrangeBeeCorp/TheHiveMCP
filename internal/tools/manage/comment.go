...
func (t *Tool) handleComment(ctx context.Context, params *EntityParams) (EntityResult, error) {
...
func (t *Tool) addComment(ctx context.Context, client *thehive.APIClient, entityType, entityID, comment string) SingleEntityCommentResult {
	switch entityType {
	case types.EntityTypeCase:
		...
		return SingleEntityCommentResult{
			...
			Result:    utils.TrustedString("comment added with ID " + result.GetUnderscoreId()),
		}
	...
	case types.EntityTypeTask:
		...
		return SingleEntityCommentResult{
			EntityID:  entityID,
			CommentID: result.GetUnderscoreId(),
			Result:    utils.TrustedString("task log added with ID " + result.GetUnderscoreId()),
		}
