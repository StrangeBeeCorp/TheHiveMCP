...
func (t *Tool) executeQuery(ctx context.Context, hiveQuery thehive.InputQuery, entityType string) ([]map[string]any, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to search %ss: %w. Check that you have permissions to view %ss. API response: %s", entityType, err, entityType, utils.DescribeHTTPResponse(resp))
	}
