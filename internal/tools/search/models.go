...
type EntitiesParams struct {
	...
	ExtraColumns      []string       `json:"extra-columns,omitempty"      jsonschema_description:"Extra columns to return ON TOP OF the entity defaults, which are always included — you never need to re-list a default to keep it. Defaults are entity-specific: alerts include severity/status, cases include status/severity, tasks include assignee, observables include dataType/data. Query the [entity]-schema from server resources for available columns."`
