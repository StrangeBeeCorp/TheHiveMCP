...
- severity is numeric, 1=Low 2=Medium 3=High 4=Critical by default, but the range and labels are configurable per organisation — read severityLabel rather than assuming the mapping. status/stage are strings.
...
// Sort orders and the default result window accepted by search-entities. The
// schema advertises these and validation applies them, so they are declared
// once rather than repeated as literals in both places.
const (
	SortOrderAsc  = "asc"
	SortOrderDesc = "desc"
...
type EntitiesParams struct {
	...
	ExtraColumns      []string       `json:"extra-columns,omitempty"      jsonschema_description:"The exact set of columns to return. Despite the name this REPLACES the entity defaults rather than adding to them, so list every column you want, including the ones you would have got for free. Omit it to keep the defaults, which are entity-specific: alerts include severity/status, cases include status/severity, tasks include assignee, observables include dataType/data. Query the [entity]-schema from server resources for available columns."`
