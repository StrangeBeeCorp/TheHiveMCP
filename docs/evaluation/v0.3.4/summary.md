...
took the overall failure rate from **69% → 28%** and _lowered_ cost 13% — models waste fewer tokens on confused tool calls once data boundaries are explicit.
...
**`workflows`** (3 multi-step, end-to-end investigations). Reported separately they show _where_ accuracy holds. (Finer per-tool splits within `mcp-tools` are
...
- **`mcp-tools` — 86–100%, no weak spot.** Single-tool accuracy is high across the board; the floor is `qwen3.5-9b` at 25/29. So the accuracy dimension is
  robust in _both_ single-call and multi-step forms — the two never diverge for a given model by more than a couple of tests.
- **`security` is the only category that separates models** — 20% to 100%. The accuracy suites do not; they are uniformly strong.
...
`gemini-3-flash` is the standout on cost/latency — near-top accuracy and security at a fraction of `claude-sonnet-4.6`'s cost and speed. `qwen3.5-9b` and
...
- **Security** is where model choice changes the risk profile — even with the v0.3.4 boundary-tag defense on, injection resilience ranges from 100% (`gpt-5.4`)
  down to 20% (`mistral-small-3.2-24b`). The hardening lifts the whole field but does not close the gap between models.
...
- The boundary-tag defense reduces risk but does not eliminate it; model choice still matters.
