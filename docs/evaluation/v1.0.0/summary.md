...
- **MCP-server version:** v1.0.0 (released July 6, 2026)
- **Evaluated:** July 2, 2026
...
This run's results reflect the model set, not the server:

- **Accuracy is saturated across the recommended matrix.** Eight of 10 models score 37/37. The two smallest (`qwen3.5-9b`, `ministral-8b-2512`) are the only
  ones below 100%.
...
and **`workflows`** (3 multi-step, end-to-end investigations). Reported separately they show where accuracy holds. (Finer per-tool splits within `mcp-tools`
aren't retained for these runs.) Full counts are in [`results.csv`](results.csv).
...
- **`mcp-tools` — 68–100%.** Single-tool accuracy is high for eight of 10 models. The floor is the two smallest models (`ministral-8b-2512` 23/34, `qwen3.5-9b`
  24/34), which account for essentially all accuracy misses in the matrix.
- **`security` is the only category that separates the field** — 53% to 100%. As in prior evaluations, the accuracy suites don't separate models. Injection
  resilience does.
...
`gemini-3.1-flash-lite` offers the best cost/latency tradeoff: perfect accuracy and near-top security (14/15) at the lowest latency and near-lowest cost in the
...
- The boundary-tag defense reduces injection risk but doesn't eliminate it. Model choice still matters, and no model should be assumed resilient outside the
  tested scenarios.
- Provider-side regressions between runs aren't detected (see [ADR-0001](../../explanation/adr/0001-security-and-accuracy-testing-policy.md), "what we don't
  test").
