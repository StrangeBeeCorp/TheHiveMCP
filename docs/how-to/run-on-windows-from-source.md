...
Compile a stable `.exe` a single time, then point your MCP host at it. This is the source path we recommend: the binary is built once, has a fixed location, and
starts instantly.
...
Then configure your host to launch that binary directly — for example, in Claude Code's `~/.claude.json`:
...
For the full Claude Code walkthrough (config precedence, verification, troubleshooting), follow
...
`go run` compiles to a throwaway binary and executes it in one step. It is convenient while hacking on the server, but **do not configure a host to launch a
...
- **No pinned version with `@latest`.** `go run …@latest` re-resolves to whatever the newest tag is at each launch, fetched over the network. Silently floating
  a security tool to the latest upstream on every start is not a posture you want in production.
