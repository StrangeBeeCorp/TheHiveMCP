...
- **Docker (recommended).** No unsigned-binary prompts, works identically from the terminal, VS Code, and any other MCP host. This is the path this guide uses.
...
clean run with no `ERROR` line about credentials means you're good.
...
> Claude Code can't reach. `--rm` cleans up the container when the session ends.

---

## Step 2 — Register the server with Claude Code

Use `claude mcp add`. It writes a syntactically valid entry for you, so you never hand-edit Claude Code's large global state file.
...
> `~/.claude.json`, which also holds unrelated Claude Code state (startup count, history, …) and is awkward to edit or review by hand.
...
- Re-run the Step 1 verification command — it isolates whether the problem is Docker, credentials, or Claude Code's config.
